...
namespace LandaJune
{
	namespace Parameters
	{
		...
		{
			explicit ABSTRACT_INPUT(const FrameProviders::FrameRef * frame) 
				: _frame(frame)
				, _Pixel2MM_X(frame->getBatchParams()->Pixel2MM_X())
				, _Pixel2MM_Y(frame->getBatchParams()->Pixel2MM_Y())
			{}

			const FrameProviders::FrameRef * _frame = nullptr;
		...
		{
			PARAMS_PAPEREDGE_INPUT(const FrameProviders::FrameRef * frame, const SHEET_SIDE side) 
		...
		{
			explicit PARAMS_I2S_INPUT(const FrameProviders::FrameRef * frame, const SHEET_SIDE side) 
				: ABSTRACT_INPUT(frame)
				, _side(side)
			{}
		...
		{
			explicit PARAMS_C2C_ROI_INPUT(const FrameProviders::FrameRef * frame, const SHEET_SIDE side, std::vector<HSV> hsvs, const ROIRect& roi, int roiIndex)
				: ABSTRACT_INPUT(frame)
				, _side(side)
		...
		{
			explicit PARAMS_WAVE_INPUT(const FrameProviders::FrameRef * frame)
				: ABSTRACT_INPUT(frame)
			{}
		...
		struct PARAMS_C2C_STRIP_INPUT : ABSTRACT_INPUT
		{
			...
			explicit PARAMS_C2C_STRIP_INPUT(const FrameProviders::FrameRef * frame, const SHEET_SIDE side)
				: ABSTRACT_INPUT(frame)
				, _side(side)
		...
		{
			explicit PARAMS_C2C_SHEET_INPUT(const FrameProviders::FrameRef * frame)
				: ABSTRACT_INPUT(frame)
