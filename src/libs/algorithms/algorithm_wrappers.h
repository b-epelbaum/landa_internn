...
namespace LandaJune
{
	namespace Algorithms
	{
		...
		PARAMS_C2C_SHEET_OUTPUT ALGORITHMS_EXPORT calculateAll(const FrameProviders::FrameRef* frame);
		void ALGORITHMS_EXPORT clearAlgorithmsData();

		// todo : move function to functions library
		void generateRegions(const FrameProviders::FrameRef* frame, PARAMS_C2C_SHEET_INPUT& input);
		PARAMS_C2C_SHEET_OUTPUT calculateSheet(const PARAMS_C2C_SHEET_INPUT& sheetInput);
		PARAMS_C2C_STRIP_OUTPUT calculateStrip (const PARAMS_C2C_STRIP_INPUT& stripInput);
