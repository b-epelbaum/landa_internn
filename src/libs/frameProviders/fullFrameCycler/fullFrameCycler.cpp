...
#define CYCLIC_GENERATOR_SCOPED_LOG PRINT_INFO4 << "[fullFrameCycler] : "
#define CYCLIC_GENERATOR_SCOPED_ERROR PRINT_ERROR << "[fullFrameCycler] : "
#define CYCLIC_GENERATOR_SCOPED_WARNING PRINT_WARNING << "[fullFrameCycler] : "

static const QString FULL_FRAME_CYCLER_PROVIDER_NAME = "Full Frame Cycler";
static const QString FULL_FRAME_CYCLER_DESC = "Full Frame Cycler generates full frame image in specified time period";


fullFrameCycler::fullFrameCycler()
{
	_name = FULL_FRAME_CYCLER_PROVIDER_NAME;
	_description = FULL_FRAME_CYCLER_DESC;
...
void fullFrameCycler::init(BaseParametersPtr parameters, Core::ICore * coreObject, CoreEventCallback callback)
{
	try
	{
...
void fullFrameCycler::validateParameters(BaseParametersPtr parameters)
{
	...
	CYCLIC_GENERATOR_SCOPED_LOG << "_ImageMaxCount = " << _ImageMaxCount;

	_providerParameters = parameters;
}

void fullFrameCycler::cleanup()
{
	try
	{
...
int32_t fullFrameCycler::getFrameLifeSpan() const
{
...
CORE_ERROR fullFrameCycler::prepareData(FrameRef* frameRef)
{
	if ( _sourceTemplateImage.empty() )
...
CORE_ERROR fullFrameCycler::accessData(FrameRef* frameRef)
{
...
void fullFrameCycler::releaseData(FrameRef* frameRef)
{
}
