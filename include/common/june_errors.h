...
namespace LandaJune
{
	...
	*/
	...
	#define ADD_ERR_DESC(x) _errorStringMap[x] = #x;
	...
	public:
		...
		inline static std::map<int, std::string> _errorStringMap;
		static void initStringMap()
		{
			_errorStringMap[ERR_PROVIDER_INVALID_SELECTED_PROVIDER] =				"Selected provider is invalid";
			_errorStringMap[ERR_FRAMEGRABBER_LOAD_APPLET_FAILED] = 					"Failed loading Silicon Software applet";
			_errorStringMap[ERR_FRAMEGRABBER_LOAD_CONFIG_FAILED] =					"Failed loading Silicon Software configuration file";
			_errorStringMap[ERR_FRAMEGRABBER_CANNOT_GET_PROPERTY] =					"Cannot get named property";
			_errorStringMap[ERR_CORE_ALGO_RUNNER_THROWN_RUNTIME_EXCEPTION] = 		"Runtime exception has been caught";
			
			ADD_ERR_DESC(ERR_FRAMEGRABBER_MEMORY_ALLOCATION_FAILED);
			
			ADD_ERR_DESC(ERR_FRAMEGRABBER_IMAGE_ACQUISITION_FAILED); 	
			ADD_ERR_DESC(ERR_FRAMEGRABBER_IMAGE_TIMEOUT	);			
			ADD_ERR_DESC(ERR_FRAMEGRABBER_IMAGE_INVALID_DATA_POINTER);
			ADD_ERR_DESC(ERR_FRAMEGRABBER_IMAGE_SKIPPED);			
			ADD_ERR_DESC(ERR_FGSIMULATOR_NO_FILE_FOUND);				
			ADD_ERR_DESC(ERR_OFFLINEREADER_SOURCE_FOLDER_INVALID);	
			ADD_ERR_DESC(ERR_OFFLINEREADER_SOURCE_FILE_INVALID);		
			ADD_ERR_DESC(ERR_OFFLINEREADER_NO_MORE_FILES);			
			ADD_ERR_DESC(ERR_SIMULATOR_HAVE_NO_IMAGES);				
			ADD_ERR_DESC(ERR_SIMULATOR_REACHED_MAX_COUNT);			
			ADD_ERR_DESC(ERR_PROVIDER_INVALID_PARAMETERS);			
			ADD_ERR_DESC(ERR_PROVIDER_DLL_CANNOT_BE_LOADED);			
			ADD_ERR_DESC(ERR_RUNNER_DLL_CANNOT_BE_LOADED);

			ADD_ERR_DESC(ERR_CORE_NOT_INITIALIZED);						
			ADD_ERR_DESC(ERR_CORE_NO_PROVIDER_SELECTED);					
			ADD_ERR_DESC(ERR_CORE_NO_ALGORITHM_RUNNER_SELECTED);			
			ADD_ERR_DESC(ERR_CORE_PROVIDER_IS_BUSY);						
			ADD_ERR_DESC(ERR_CORE_PROVIDER_THROWN_RUNTIME_EXCEPTION);		
			ADD_ERR_DESC(ERR_CORE_PROVIDER_FAILED_TO_INIT);				
			ADD_ERR_DESC(ERR_CORE_CANNOT_CREATE_FOLDER);					
			ADD_ERR_DESC(ERR_CORE_CANNOT_ENCODE_TO_BMP);					
			
			ADD_ERR_DESC(ERR_FRAME_INVALID_INIT_DATA);					
			ADD_ERR_DESC(ERR_FRAME_INVALID_IMAGE_FORMAT);				
			ADD_ERR_DESC(ERR_FRAME_INVALID_FRAME_IMAGE_DIMS);				
			ADD_ERR_DESC(ERR_FRAME_INVALID_BATCH_PARAMS);					
			ADD_ERR_DESC(ERR_FRAME_NO_FRAME_REGIONS);						
			ADD_ERR_DESC(ERR_FRAME_DIFFERENT_EXPECTED_SIZE);				
			ADD_ERR_DESC(ERR_FRAME_INSUFFICIENT_BUFFER_SIZE);				
			
			ADD_ERR_DESC(ALGO_ROI_RECT_IS_EMPTY);							
			ADD_ERR_DESC(ALGO_ROI_INVALID_RECT); 							
			ADD_ERR_DESC(ALGO_ROI_RECT_EXCEEDS_FRAME_RECT); 				
			ADD_ERR_DESC(ALGO_INVALID_SOURCE_IMAGE);						
			ADD_ERR_DESC(ALGO_EMPTY_ROI_NAME_TO_SAVE);					
			ADD_ERR_DESC(ALGO_INIT_EDGE_FAILED);							
			ADD_ERR_DESC(ALGO_INIT_I2S_FAILED);							
			ADD_ERR_DESC(ALGO_INIT_C2C_FAILED);							
			ADD_ERR_DESC(ALGO_INIT_WAVE_FAILED);							
			ADD_ERR_DESC(ALGO_SHUTDOWN_EDGE_FAILED);						
			ADD_ERR_DESC(ALGO_SHUTDOWN_I2S_FAILED);						
			ADD_ERR_DESC(ALGO_SHUTDOWN_C2C_FAILED);						
			ADD_ERR_DESC(ALGO_SHUTDOWN_WAVE_FAILED);						
			ADD_ERR_DESC(ALGO_PROCESS_EDGE_FAILED);						
			ADD_ERR_DESC(ALGO_PROCESS_I2S_FAILED);						
			ADD_ERR_DESC(ALGO_PROCESS_C2C_FAILED);						
			ADD_ERR_DESC(ALGO_PROCESS_WAVE_FAILED);						
		}
